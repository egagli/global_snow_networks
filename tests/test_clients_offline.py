...
# ── the shared type vocabulary (DESIGN.md §3.2) ───────────────────────────────
...
def test_every_client_uses_only_the_shared_type_vocabulary():
    ...
    sensors used it. It lives in `clients/_common.py` now, like INTERVALS.
    """
    import importlib

    from clients._common import TYPES

    offenders = {}
    for name in ("awdb", "cdec", "databc", "nve", "yukon"):
        module = importlib.import_module(f"clients.{name}.{name}_client")
...
def test_snowfall_is_not_filed_under_precipitation():
    ...
    of water.
    """
    from clients.yukon.yukon_client import VARIABLES, _TYPE_TO_YUKON_VARS
